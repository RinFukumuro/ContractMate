// Quick Word Document Navigator
// Windows Script Host (WSH) + ActiveXObject
// This version does not use infinite loops and exits automatically
...
function main() {
    ...
    try {
        ...
        if (args.length < 1) {
            WScript.Echo("Usage: cscript quick_navigator.js [Word document path] [paragraph number (optional)]");
            WScript.Quit(1);
        }
        ...
        // Keep Word open but exit script
        WScript.Echo("\nWord document is now open. You can continue working with it.");
        WScript.Echo("Close Word manually when finished.");
        
        // Don't close Word or the document - let the user do it manually
        // Just exit the script
        word = null; // Release the object reference
